test_that("CodeSiteEff_l2_par works with valid input", {
  ...
  expect_true(all(c("delta", "V.1.new", "V.2.new") %in% names(result)))
  expect_equal(dim(result$delta), c(n1 + n2, p))
  expect_equal(dim(result$V.1.new), c(n1, p))
  expect_equal(dim(result$V.2.new), c(n2, p))
})
